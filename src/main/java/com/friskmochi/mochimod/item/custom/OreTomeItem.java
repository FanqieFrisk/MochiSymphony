...
public class OreTomeItem extends Item {
    public OreTomeItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient()) {
            ...
            if (!Screen.hasShiftDown()) {
                ...
                if (!foundBlock) {
                    player.sendMessage(Text.of("【§e模糊搜索§r】在11*11的范围内未发现矿石！"), true);
                }
            } else {
                // 实现精确搜索
                for (int i = 0; i <= pos.getY() + 64; i++) {
                ...
                if (!foundBlock) {
                    player.sendMessage(Text.of("【§a精确搜索§r】在3*3的范围内未发现矿石！"), true);
                }
            }
    ...
    private boolean isRightBlock(BlockState blockState) {
        if (blockState.isIn(ModBlockTags.ORES)) {
            return true;
        }else {
            return false;
        }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        if (Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("item.mochisymphony.ore_tome.shift_tooltip"));
        } else {
            tooltip.add(Text.translatable("item.mochisymphony.ore_tome.tooltip"));
        }
    }
}